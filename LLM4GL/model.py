...
class RGTLayer(torch.nn.Module):
    ...
    def forward(self, events_pair, mh_self_attention_mask):#mh_self_attention_mask屏蔽矩阵
...
class LongformerCausalModel(nn.Module):
    ...
    def forward(self, enc_input_ids, enc_mask_ids, global_attention_mask, t1_pos, t2_pos, mh_self_attention_mask, target,length):
        ...
        target = torch.cat([torch.tensor(t) for t in zip(target)], dim=0).to(self.device)
        
        l=length[0]
        opt=opt[:l]
       
        # opt = torch.cat([o[:l] for o, l in zip(opt, length)], dim=0)
